  ...
  describe("/auth", () => {
    ...
    describe("should call discord oAuth service", () => {
      ...
      it("for getting user token respond 401 discord authent fail", async () => {
        fakeServer.spies.response.mockImplementation((_req, res) =>
          res.status(401).send()
        );
      ...
      it("for getting user token respond 401 when user is not in server", async () => {
        const userDiscord = {
        ...
        fakeServer.spies.response
          ...
          .mockImplementationOnce((_req, res) =>
            res.status(200).json(userDiscord)
          )
          .mockImplementationOnce((_req, res) =>
      ...
      it("and return user jwt when all step are done", async () => {
        ...
        expect(response.data.token).toMatchJWT({
          ...
          pseudo: `${userDiscord.username}#${userDiscord.discriminator}`
        });
      });
    });
  });
